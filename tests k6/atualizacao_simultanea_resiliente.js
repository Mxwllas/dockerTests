    ...
    for (let i = 1; i <= total; i += batchSize) {
        ...
        for (let r = 0; r < responses.length; r++) {
            if (responses[r].status === 201 || responses[r].status === 200) {
                ...
                console.error(`Falha ao criar usuário ${i + r}: status ${responses[r].status}`);
            }
        }
    }
    // Confirma se um número suficiente de usuários foi criado (pelo menos 80%)
    if (idsCriados.length < total * 0.8) {
        throw new Error(`Número insuficiente de usuários criados: ${idsCriados.length} de ${total}`);
    }
...
export default function (data) {
    const ids = data.ids;
    if (ids.length === 0) {
        console.log('Nenhum usuário criado no setup, pulando iteração.');
        return;
    }
